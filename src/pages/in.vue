  ...
  <el-card>
    ...
    <el-form class="forms" :model="inForm">
      ...
      <el-form-item label="存放仓库" label-width="80px">
        <el-select class="mid" label="存放仓库" label-width="80px" v-model="inForm.store" placeholder="请选择">
          <el-option
    ...
    <el-table class="tables" :data="tableData" border>
      ...
      <el-table-column label="入库时间" prop="ptime" :formatter="dateFormat"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    submit() {
      axios.post("http://localhost:8088/api/prod/ware",
        Qs.stringify({
    ...
    query() {
      ...
      axios.post("http://localhost:8088/api/wares",
        Qs.stringify({
          start: this.inTime[0],
          end: this.inTime[1]
        }), {}
